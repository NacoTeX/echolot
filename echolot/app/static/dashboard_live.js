...
async function startLiveDashboard() {
  ...
  if (generation === liveGeneration && !fusionTimer) {
    fusionTimer = setInterval(refreshFusion, 2000);
  }
}

function fusionStateLabel(result) {
  if (!result.available) return "keine Direktdaten";
...
async function refreshFusion() {
  ...
  for (const result of results) {
    ...
    if (!tile) continue;
    const state = tile.querySelector("[data-zone-state]");
    const dot = tile.querySelector("[data-dot]");
    if (state) {
      state.textContent = fusionStateLabel(result);
      state.className = `tile-state ${
        result.state === "occupied" ? "status-ok" :
        result.state === "uncertain" ? "status-warn" : "status-pending"
      }`;
    }
    if (dot && result.available) {
      dot.className = `tile-dot ${
        result.state === "occupied" ? "tile-dot-on" :
        result.state === "uncertain" ? "tile-dot-holding" : "tile-dot-off"
      }`;
    }
    ...
    note.textContent = result.available
      ? `Direkt-Fusion · Übereinstimmung ${Math.round(result.agreement * 100)} %`
      : "Direkt-Fusion wartet auf aktuelle Samples";
    for (const member of result.members) {
      const chip = tile.querySelector(`[data-member="${member.device_id}"]`);
      if (!chip) continue;
      chip.title = member.reliability > 0
        ? `${Math.round(member.probability * 100)} % Präsenz · ${Math.round(member.reliability * 100)} % Verlässlichkeit · ${member.basis}`
        : "Keine aktuellen Direktdaten";
    }
  }
}
