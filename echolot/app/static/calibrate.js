  ...
  const view = {
    ...
    alignCard(room) {
      ...
      const row = (p, i, label, meta) => `<div class="zone-row cal-point ${p.role === "check" ? "check" : ""}"><span class="spot-num">${label}</span>
          ...
          <button class="btn small ghost icon-only" data-remeasure="${i}" title="Neu messen" aria-label="Punkt ${label} neu messen" ${measuring ? "disabled" : ""}>${icon("rotate")}</button>
          <button class="btn small ghost icon-only" data-drop="${i}" title="Entfernen" aria-label="Punkt ${label} entfernen" ${measuring ? "disabled" : ""}>${icon("close")}</button></div>`;
      const fitRows = fitIdx.map((i, k) => {
        ...
        const meta = after !== undefined
          ? `daneben: ${cm(before)} → ${cm(after)}${held !== undefined ? ` · ohne ihn: ${cm(held)}` : ""}` : "gemessen";
      ...
      const checkRows = checkIdx.map((i, k) => {
        ...
        return row(points[i], i, `K${k + 1}`, e !== undefined ? `Kontrolle: ${cm(e)} daneben` : "gemessen");
      }).join("");
      ...
      if (measuring) {
        ...
        const label = this.remeasure !== null && this.remeasure !== undefined
          ? "Punkt neu messen"
    ...
    finished() {
      ...
      if (cap.kind === "point") {
        ...
        this.measuring = null;
        if (r && r.ok && ref) {
          const entry = { ref, raw: r.raw, warnings: r.warnings, spread_m: r.spread_m, share: r.share,
            role: this.remeasure !== null && this.remeasure !== undefined ? this.points()[this.remeasure].role : this.pendingRole || "fit" };
    ...
    async solve() {
      ...
      try {
        ...
        this.proposal = await api(`api/rooms/${encodeURIComponent(this.id)}/alignment`, { method: "POST",
          body: { points: points.map((p) => ({ raw: p.raw, ref: p.ref, role: p.role || "fit" })), mount_height_m: m.mount_height_m, target_height_m: m.target_height_m } });
